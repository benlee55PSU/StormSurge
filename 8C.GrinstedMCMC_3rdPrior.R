...
sdvect<-c(rep(1,3))  # Prior Distribution Parameter 2: Does not matter for Uniform Priors
#MCMC
...
save(run2,file="Prior3_Grinsted_Stat_Output.RData") # Save Data
...
sdvect<-c(rep(1,4))  # Prior Distribution Parameter 2: Does not matter for Uniform Priors
#MCMC
run1<-MCMC_breaks_resume(10000,2500,res=FALSE,resdat=NA,datset,tempset,
                         start,errvect,uvect,sdvect,filetitle="Test1") # First Run
run2<-MCMC_breaks_resume(10000,2500,res=TRUE,resdat=run1,datset,tempset,
...
MULTncrej(run2,burn=burnin) #RejectionRate
cred.table<-CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
save(run2,file="Prior3_Grinsted_Mu_Output.RData") # Save Data
...
sdvect<-c(rep(1,5))  # Prior Distribution Parameter 2: Does not matter for Uniform Priors
...
MULTncrej(run2,burn=burnin) #RejectionRate
cred.table<-CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
save(run2,file="Prior3_Grinsted_MuSigma_Output.RData")

###########################################################################################
# (4) Fully Non-Stationary
source("/Users/su_skl5261/Box Sync/FinalPaper/DataRepository/SourceCode/Prior2Source.R") # Load Source File
# Initial Conditions
...
sdvect<-c(rep(1,6))  # Prior Distribution Parameter 2: Does not matter for Uniform Priors
...
MULTncrej(run2,burn=burnin) #RejectionRate
cred.table<-CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
save(run2,file="Prior3_Grinsted_FullNS_Output.RData") # Save Data
###########################################################################################
