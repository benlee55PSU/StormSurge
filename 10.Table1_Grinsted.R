...
statpars<-gev.fit(xdat=datset,siglink=exp)$mle #MLE estimates
# AIC
...
statbic 
# DIC
statpars<-CredIntervalsGEV(run2,burn=burnin)[,2] #Posterior Mean
burnin=6000 # Burnin
stat_posteriordraws<-run2[[1]][-(1:burnin),]
...
#################################################################################################
# (2) Mu Only Non-stationary
source("/Users/su_skl5261/Box Sync/FinalPaper/SourceCode/UniformSourceMu.R")
...
# DIC
burnin=6000 # Burnin
mupars<-CredIntervalsGEV(run2,burn=burnin)[,2] # Bayes Posterior Mean
mu_posteriordraws<-run2[[1]][-(1:burnin),]
...
musigmapars<-c(musigma.mle$mle[c(1,3,5)],
               (musigma.mle$mle[2]/musigma.mle$mle[1]),(musigma.mle$mle[4]/musigma.mle$mle[3]))
...
fulldev_musigma.orig<-fulldev_musigma
fulldev_musigma<-fulldev_musigma.orig
