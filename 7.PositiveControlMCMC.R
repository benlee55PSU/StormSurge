...
uvect<-c(rep(NA,3)) # Does not matter
sdvect<-c(rep(NA,3))  # Does not matter
...
save(statrun2,datset,tempset,file="PositiveControl_Stat_Output.RData")
...
uvect<-c(rep(NA,4)) # Does not matter
sdvect<-c(rep(NA,4))  # Does not matter
#Testing for resuming MCMC
...
save(murun2,datset,tempset,file="PositiveControl_Mu_Output.RData")
...
uvect<-c(rep(NA,5)) # Does not matter
sdvect<-c(rep(5,5))  # Does not matter
#Testing for resuming MCMC
...
save(run2,datset,tempset,file="PositiveControl_MuSigma_Output.RData")
...
uvect<-c(rep(NA,6)) # Does not matter
sdvect<-c(rep(5,6))  # Does not matter
#Testing for resuming MCMC
run1<-MCMC_breaks_resume(50000,2500,res=FALSE,resdat=NA,datset,tempset,
                         start,errvect,uvect,sdvect,filetitle="Test1") # First Run
run2<-MCMC_breaks_resume(50000,2500,res=TRUE,resdat=run1,datset,tempset,
...
MULTncrej(run2,burn=burnin) #RejectionRate
CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
save(run2,datset,tempset,file="PositiveControl_FullNS_Output.RData")
